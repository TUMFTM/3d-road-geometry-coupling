...
//
// Standalone executable that constructs `RoadGeometryCouplerNode` directly
// from a TrackData CSV (no `track_handler_cpp` configuration needed).
// Bypasses the rclcpp_components wrapper and is the entry point used by the
// bundled demo launch file.
#include <memory>
...
int main(int argc, char ** argv)
{
  ...
  auto bootstrap = std::make_shared<rclcpp::Node>("RoadGeometryCoupler", options);
  ...
  if (track_csv_path.empty()) {
    RCLCPP_FATAL(
      ...
      "road_geometry_coupler_node_from_csv: parameter 'track_csv_path' must be set, "
  ...
  auto node =
    std::make_shared<tam::road_geometry_coupling::RoadGeometryCouplerNode>(options, track_data);
